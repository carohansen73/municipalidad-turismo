@extends('layouts.front')
 ...
 @section('title', $serviceProvider->title)
            ...
            <div class="row justify-content-center">
                    ...
                    <div class="single-page-post">
                        <div class="top-wrapper ">
                            <div class="row d-flex justify-content-between">
                                ...
                                {{-- <div class="col-lg-4 col-md-12 right-side d-flex justify-content-end">
                                    <div class="desc">
                                       <h2><i class="fa fa-folder-open"></i> {!! $serviceProvider->location_city !!}</h2>
                                        <h3> {!! $serviceProvider->location_address !!} </h3>
                                    </div>
                                </div> --}}
                            </div>
                        </div>
                            ...
                            <img class="img-fluid" src="{!!url('storage/services/'. $serviceName .'/' . $serviceProvider->lastImage->photo_url) !!}" alt="">



                        {{-- <div class="tags">
                            <ul>
                                @foreach ($serviceProvider->amenities as $amenitie)
                                    <li><a href="">{!! $amenitie->name !!}</a></li>
                                @endforeach
                            </ul>
                        </div> --}}
                        ...
                        <div class="single-post-content">
                            <blockquote class="generic-blockquote">
                                {!! $serviceProvider->location_city !!} -
                                {!! $serviceProvider->location_address !!}
                            </blockquote>


                            <p> {!! $serviceProvider->description !!} </p>
                            <br>
                            <h3 style="color:#777;"> Comodidades </h3>
                            <br>
                            @foreach ($amenities as $amenitie)
                            <p> <span class="amenities-icon"> <i class="{!! $amenitie->icon !!}"></i> </span>  {!! $amenitie->name !!} </p>
                            @endforeach
                                        ...
                                        @foreach($suggestions as $sp)
                                            <div class="col-sm-6 nav-left justify-content-start d-flex">
                                                ...
                                                <div class="details">
                                                    ...
                                                    <h4 class="text-uppercase"><a href="{!! route('proveedor.ver', [$sp->id]) !!}" aria-label="Anterior noticia noticia">{!! $sp->title !!}</a></h4>
                                                </div>
                                            </div>
                ...
                <div class="col-lg-4 sidebar-area ">
                    ...
                    <div class="single_widget recent_widget">
                        <h4 class="text-uppercase pb-20">Galería de imágenes </h4>
                        <div class="active-recent-carusel">
                            @foreach ($images as $img)
                                <div class="item">
                                    <img src="{!! url('storage/services/'. $serviceName .'/' . $img->photo_url) !!}" alt="">
                    ...
                    <div class="single_widget">
                        <h3 class="text-heading">Información de contacto</h3>
                        <p><i class="fa fa-calendar"></i> {!! $serviceProvider->contact_name !!}</p>
                        @if($serviceProvider->contact_email )
                            <p><i class="fa fa-at"></i> {!! $serviceProvider->contact_email !!}</p>
                        @endif
                        @if($serviceProvider->contact_ig )
                        <p><i class="fa fa-instagram"></i> {!!$serviceProvider->contact_ig !!}</p>
                        @endif
                        @if($serviceProvider->contact_fb )
                        <p><i class="fas fa-facebook"></i> {!!$serviceProvider->contact_fb !!}</p>
                        @endif
                        @if($serviceProvider->contact_web )
                        <p><i class="fa fa-globe"></i> {!!$serviceProvider->contact_web !!}</p>
                        @endif
